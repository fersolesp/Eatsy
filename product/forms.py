...
from .models import Reporte, CausaReporte, Producto, Ubicacion, Aportacion, Dieta
...
class AddDietForm(forms.ModelForm):
    ...
    def __init__(self, dietas, *args, **kwargs):
        super(AddDietForm, self).__init__(*args, **kwargs)
        self.fields['dietas'].choices = dietas
    dietas = forms.MultipleChoiceField(label='Etiqueta', choices=Dieta_Enum, widget=forms.SelectMultiple(attrs={'class' : 'form-control', 'style':'width : 350px'}),required=True)
    class Meta:
        model = Dieta
        fields = ['nombre']