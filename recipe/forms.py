...
class CreateRecipeForm(forms.Form):
    ...
    productos_choices = [(p.id, p.titulo) for p in Producto.objects.all().order_by('titulo')]
    productos = forms.MultipleChoiceField(label='Selecciona un género', choices=productos_choices, widget=forms.SelectMultiple(attrs={'class' : 'form-control', 'style':'width : 100%; height: 15em;'}))